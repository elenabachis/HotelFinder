...
public class FeedbackRepo {
    // SQL queries
    private static final String INSERT_FEEDBACK_SQL = "INSERT INTO feedback (user_id, hotel_id, services_rating, cleanliness_rating, comments) VALUES (?, ?, ?, ?, ?)";
    private static final String SELECT_FEEDBACK_BY_USER_HOTEL_SQL = "SELECT * FROM feedback WHERE user_id = ? AND hotel_id = ?";
    private static final String SELECT_ALL_FEEDBACKS_SQL = "SELECT * FROM feedback";
    private static final String UPDATE_FEEDBACK_SQL = "UPDATE feedback SET services_rating = ?, cleanliness_rating = ?, comments = ? WHERE user_id = ? AND hotel_id = ?";
    private static final String DELETE_FEEDBACK_SQL = "DELETE FROM feedback WHERE user_id = ? AND hotel_id = ?";
    ...
    public void updateFeedback(Feedback feedback) {
        try (Connection connection = DriverManager.getConnection(DbConfigurator.URL, DbConfigurator.USERNAME, DbConfigurator.PASSWORD);
            ...
            preparedStatement.setInt(5, feedback.getHotelId());
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
